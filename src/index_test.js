...
// constantswdw
const DEFAULTS = { debug: false, maxItems: 10, tag: "demo" };
const EMPTY_FN = () => {};
const NULL_OBJ = {};
...
function isLowerCaseFirst(s) {
  return typeof s === "string" && s[0] === s[0]?.toLowerCase();
}
...
function getPath(obj, path, def) {
  ...
  return (
    String(path)
      .split(".")
      .reduce((acc, key) => (acc == null ? undefined : acc[key]), obj) ?? def
  );
}
